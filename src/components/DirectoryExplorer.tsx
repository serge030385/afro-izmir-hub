...
import { categories, categoryLabels, professionals } from "@/lib/data";
...
export function DirectoryExplorer() {
  ...
  const filtered = useMemo(() => {
    ...
    return professionals.filter((professional) => {
      const matchesCategory = category === "Toutes" || professional.category === category;
      const matchesQuery =
        !term ||
        [professional.name, professional.category, categoryLabels[professional.category], professional.description, professional.city]
          .map((value) => (typeof value === "string" ? value : tx(value)))
          .join(" ")
          .toLowerCase()
          .includes(term);
    ...
    <div>
      <div className="mb-8 grid gap-3 rounded-lg border border-black/10 bg-white p-4 shadow-sm md:grid-cols-[1fr_260px]">
        <label className="relative block">
          ...
          <input
            ...
            placeholder={t("searchProfessional")}
      ...
      {filtered.length > 0 ? (
        ...
        <div className="rounded-lg border border-dashed border-black/20 bg-white p-8 text-center">
          <p className="font-bold text-zinc-950">{t("noProfessional")}</p>
